...
const updateResource = async (UpdateResource)=>{
  ...
  if (!res.ok) {
    ...
    // Throw an error with the message returned from the controller
...
function EditResource({resourceEdite ,onChangeActiveTabe}) {
  ...
  const [updateDescription, setUpdateDescription] = useState(resourceEdite.description);
            ...
            <div>
              ...
              <textarea
               name=""
                ...
                value={updateDescription}></textarea>
            </div>
          </div>
          <button 
